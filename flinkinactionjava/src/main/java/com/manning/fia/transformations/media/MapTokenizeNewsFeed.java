package com.manning.fia.transformations.media;

import com.manning.fia.c03.media.DateUtils;
import com.manning.fia.model.media.NewsFeed;

import org.apache.flink.api.common.functions.MapFunction;
import org.apache.flink.api.java.tuple.Tuple5;

@SuppressWarnings("serial")
public class MapTokenizeNewsFeed implements MapFunction<String, Tuple5<String, String, String, Long,Long>> {
    private DateUtils dateUtils = new DateUtils();
    @Override
    public Tuple5<String, String, String, Long,Long> map(String value) throws Exception {
        final NewsFeed newsFeed=NewsFeedParser.mapRow(value);
        final long timeSpent = dateUtils.getTimeSpentOnPage(newsFeed);
        final Tuple5<String, String, String, Long,Long> tuple5 = new Tuple5<>(newsFeed.getSection(), newsFeed
                .getSubSection(), newsFeed.getTopic(), timeSpent,newsFeed.getPageId());
        return tuple5;
    }
}
