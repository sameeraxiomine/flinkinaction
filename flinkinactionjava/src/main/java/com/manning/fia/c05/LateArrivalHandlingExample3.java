...
public class LateArrivalHandlingExample3 {
	public static void main(String[] args) throws Exception{
		...
		DataStream<Tuple4<Integer, Integer,Integer,Long>> selectDS = eventStream.map(new IdentityMapper<Tuple4<Integer, Integer,Integer,Long>>());

		int windowInterval = 5;
		System.out.println("Window Interval Is: " + windowInterval);
		selectDS.keyBy(0).timeWindow(Time.seconds(windowInterval)).allowedLateness(Time.milliseconds(2000)).apply(new SampleApplyFunction()).printToErr();
		execEnv.execute("Late Events Partially Lost");
	}
